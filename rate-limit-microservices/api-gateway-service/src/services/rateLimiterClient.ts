...
import type { RateLimitCheckResult, RateLimitStatus } from "../types/index.js";
  ...
  async checkLimit(
    ...
    try {
      ...
      if (
        ...
        return {
          allowed: false,
          remaining: error.response.data?.error?.remaining || 0,
          limit: error.response.data?.error?.limit || 0,
          resetIn: error.response.data?.error?.resetIn || 0,
        };
      }
